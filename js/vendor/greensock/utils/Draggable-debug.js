...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	window._gsDefine("utils.Draggable", ["events.EventDispatcher","TweenLite"], function(EventDispatcher, TweenLite) {
		...
		var _tempVarsXY = {css:{}}, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
			...
			_addListener = function(element, type, func, debug) {
                if (debug) console.log('DRAGGABLE::_addListener', element.tagName ? element.tagName : element, type, func.name);
			...
			_removeListener = function(element, type, func, debug) {
                if (debug) console.log('DRAGGABLE::_removeListener', element.tagName ? element.tagName : element, type, func.name);
			...
			_onMultiTouchDocumentEnd = function(e) {
                console.log('DRAGGABLE::_onMultiTouchDocumentEnd');
				_isMultiTouching = (e.touches && _dragCount < e.touches.length);
				_removeListener(e.target, "touchend", _onMultiTouchDocumentEnd);
			},

			_onMultiTouchDocument = function(e) {
                console.log('DRAGGABLE::_onMultiTouchDocument');
				_isMultiTouching = (e.touches && _dragCount < e.touches.length);
			...
			Draggable = function(target, vars) {
				...
				var type = (vars.type || (_isOldIE ? "top,left" : "x,y")).toLowerCase(),
					...
					onPress = function(e) {
                        if (self.myDebug) console.log('DRAGGABLE::onPress');
						var temp;
						...
						if (_touchEventLookup[e.type]) { //note: on iOS, BOTH touchmove and mousemove are dispatched, but the mousemove has pageY and pageX of 0 which would mess up the calculations and needlessly hurt performance.
							...
							_addListener(touchEventTarget, "touchend", onRelease, self.myDebug);
							_addListener(touchEventTarget, "touchmove", onMove, self.myDebug);
							_addListener(touchEventTarget, "touchcancel", onRelease, self.myDebug);
							_addListener(_doc, "touchstart", _onMultiTouchDocument, self.myDebug);
						} else {
							touchEventTarget = null;
							_addListener(_doc, "mousemove", onMove, self.myDebug); //attach these to the document instead of the box itself so that if the user's mouse moves too quickly (and off of the box), things still work.
							_addListener(_doc, "mouseup", onRelease, self.myDebug);
						}
						isClicking = (_isClickable(e.target) && !vars.dragClickables);
						if (isClicking) {
							_addListener(e.target, "change", onRelease, self.myDebug); //in some browsers, when you mousedown on a <select> element, no mouseup gets dispatched! So we listen for a "change" event instead.
							return;
						}
						if (_isOldIE) {
					...
					onMove = function(e) {
                        if (self.myDebug) console.log('DRAGGABLE::onMove');
					...
					onRelease = function(e, force) {
                        if (self.myDebug) console.log('DRAGGABLE::onRelease');
						...
						var originalEvent = e,
							...
							touches, i;
						if (touchEventTarget) {
							_removeListener(touchEventTarget, "touchend", onRelease, self.myDebug);
							_removeListener(touchEventTarget, "touchmove", onMove, self.myDebug);
							_removeListener(touchEventTarget, "touchcancel", onRelease, self.myDebug);
							_removeListener(_doc, "touchstart", _onMultiTouchDocument, self.myDebug);
						} else {
							_removeListener(_doc, "mouseup", onRelease, self.myDebug);
							_removeListener(_doc, "mousemove", onMove, self.myDebug);
						}
						dirty = false;
						if (isClicking) {
							if (e) {
								_removeListener(e.target, "change", onRelease, self.myDebug);
							}
					...
					onClick = function(e) {
                        if (self.myDebug) console.log('DRAGGABLE::onClick');
				...
				this.startDrag = function(e) {
                    if (self.myDebug) console.log('DRAGGABLE::startDrag');
					onPress(e);
					if (!self.isDragging) {
						self.isDragging = true;
						_dispatchEvent(self, "dragstart", "onDragStart");
				...
				this.endDrag = function(e) {
                    if (self.myDebug) console.log('DRAGGABLE::endDrag');
					onRelease(e, true);
				};
				this.timeSinceDrag = function() {
                    if (self.myDebug) console.log('DRAGGABLE::timeSinceDrag');
					return self.isDragging ? 0 : (_getTime() - dragEndTime) / 1000;
				};
				this.hitTest = function(target, threshold) {
                    if (self.myDebug) console.log('DRAGGABLE::hitTest');
					return Draggable.hitTest(self.target, target, threshold);
				};

				this.applyBounds = function(newBounds) {
                    if (self.myDebug) console.log('DRAGGABLE::applyBounds');
					var x, y;
				...
				this.update = function(applyBounds) {
                    if (self.myDebug) console.log('DRAGGABLE::update');
					var x = self.x,
						y = self.y;
					updateMatrix();
					if (applyBounds) {
				...
				this.enable = function() {
                    if (self.myDebug) console.log('DRAGGABLE::enable');
					var id;
					_addListener(trigger, "mousedown", onPress, self.myDebug);
					_addListener(trigger, "touchstart", onPress, self.myDebug);
					_addListener(trigger, "click", onClick, self.myDebug);
					if (!rotationMode) {
						_setStyle(trigger, "cursor", vars.cursor || "move");
					}
				...
				this.disable = function() {
                    if (self.myDebug) console.log('DRAGGABLE::disable');
					...
					_removeListener(trigger, "mousedown", onPress, self.myDebug);
					_removeListener(trigger, "touchstart", onPress, self.myDebug);
					_removeListener(trigger, "click", onClick, self.myDebug);
					if (touchEventTarget) {
						_removeListener(touchEventTarget, "touchcancel", onRelease, self.myDebug);
						_removeListener(touchEventTarget, "touchend", onRelease, self.myDebug);
						_removeListener(touchEventTarget, "touchmove", onMove, self.myDebug);
